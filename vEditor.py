...
class App(CTk.CTk):
    def __init__(self):
        ...
        self.File.set("File")
        #Edit Button
        self.Edit = CTk.CTkOptionMenu(self.TopFrame, values=["Copy", "Paste", "Cut", "Clipboard", "Select All"], width=100, command=self.EditFunc)
        self.Edit.grid(row=0, column=1, pady=10, padx=10)
        self.Edit.set("Edit")
        #Search Button
        self.Format = CTk.CTkOptionMenu(self.TopFrame, values=["Font Style", "Find & Replace"], width=100, command=self.FormatFunc)
        self.Format.grid(row=0, column=2, pady=10, padx=10)
        self.Format.set("Format")
        #View
        self.View = CTk.CTkOptionMenu(self.TopFrame, values=["Fullscreen", "Reset Zoom", "File Info"], width=100, command=self.ViewFunc)
        self.View.grid(row=0, column=3, pady=10, padx=10)
        self.View.set("View")
        #Tools
        self.Tools = CTk.CTkOptionMenu(self.TopFrame, values=["Timestamp", "View In Browser", "Google Search"], width=100, command=self.ToolsFunc)
        ...
        #Help Button
        self.Help = CTk.CTkOptionMenu(self.TopFrame, values=["Docs", "Version", "Troubleshooting", "Contact vEditor"], width=100, command=self.HelpFunc)
        ...
        if option == "Exit":
            ...
            if self.Saved == False:
                ...
                try:
                    ...
                    tkinter.messagebox.showerror("vEditor", "File has been Moved or Deleted")
        elif option == "New":
            self.File.set("File")
            ...
            try:
                ...
                tkinter.messagebox.showerror("vEditor", "Could not Open File")
            else:
                self.Entry.delete("1.0", CTk.END)
    ...
    def EditFunc(self, option):
        if option == "Paste":
            ...
            tkinter.messagebox.showinfo("vEditor", self.clipboard)
        elif option == "Cut":
            self.Edit.set("Edit")
    ...
    def ViewFunc(self, option):
        if option == "File Info":
            self.View.set("View")
            if self.Saved == False:
                tkinter.messagebox.showwarning("vEditor", "Must Save File To Retrieve Information")
            else:
...
Absolute File Path: {self.ABSpath}"""
    ...
    def ToolsFunc(self, option):
        if option == "View In Bro   wser":
            ...
            self.Tools.set("Tools")
            tkinter.messagebox.showinfo("vEditor", datetime.datetime.now())
    ...
    def HelpFunc(self, option):
        if option == "Version":
            self.Help.set("Help")
            tkinter.messagebox.showinfo("vEditor", "Version 1.0")
        elif option == "Docs":
            self.Help.set("Help")
            webbrowser.open("https://bit.ly/vEditor")
        elif option == "Contact vEditor":
            self.Help.set("Help")
            webbrowser.open("mailto:?to=vihaanmody1@gmail.com&subject=vEditor%20User")
